...
public class CustomPointer : MonoBehaviour
{
    ...
    public bool
        pointer_returns_to_center = false; //Pointer will drift to the center of the screen (Use this for joysticks)

    public bool
        instant_snapping =
            false; //If the pointer returns to the center, this will make it return to the center instantly when input is idle. Only works for joysticks

    public float center_speed = 5f; //How fast the pointer returns to the center.

    public bool
        center_lock =
            true; //Pointer graphic will be locked to the center. Also affects shooting raycast (always shoots to the center of the screen)
    ...
    public float
        deadzone_radius =
            0f; //Deadzone in the center of the screen where the pointer can move without affecting the ship's movement.
    ...
    [HideInInspector] public Rect deadzone_rect; //Rect representation of the deadzone.
    ...
    void Start()
    {
        ...
        deadzone_rect = new Rect((Screen.width / 2) - (deadzone_radius), (Screen.height / 2) - (deadzone_radius),
            deadzone_radius * 2, deadzone_radius * 2);
        ...
        if (!use_mouse_input && !use_gamepad_input)
            Debug.LogError(
                "(FlightControls) No input method selected! See the Custom Pointer script on the Main Camera and select either mouse or gamepad.");
    }
    ...
    void Update()
    {
        ...
        else if (use_gamepad_input)
        {
            ...
            pointerPosition += new Vector2(
                x_axis * thumbstick_speed_modifier * Mathf.Pow(Input.GetAxis("Horizontal"), 2),
    ...
    void OnGUI()
    {
        ...
        if (pointerTexture != null && !center_lock)
            GUI.DrawTexture(
                new Rect(pointerPosition.x - (pointerTexture.width / 2),
                    Screen.height - pointerPosition.y - (pointerTexture.height / 2), pointerTexture.width,
                    pointerTexture.height), pointerTexture);
        else
        {
            GUI.DrawTexture(
                new Rect((Screen.width / 2f) - (pointerTexture.width / 2),
                    (Screen.height / 2f) - (pointerTexture.height / 2), pointerTexture.width, pointerTexture.height),
                pointerTexture);
        }
    }
